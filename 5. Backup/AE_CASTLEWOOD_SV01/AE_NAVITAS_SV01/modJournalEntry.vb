Module modJournalEntry
    ...
    Public Function JournalEntry_Posting(ByVal oDsJournal As DataSet, ByRef oCompany As SAPbobsCOM.Company _
                                         , ByVal sFileName As String, ByRef sDocEntry As String, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            Dim dDate As Date = oDsJournal.Tables(0).Rows(0).Item(3).ToString.Trim
            Dim imonth As String = Month(dDate).ToString.PadLeft(2, "0"c)
            Dim iyear As Integer = Right(Year(dDate), 2)
            sSeries = "XN-" & CStr(imonth) & CStr(iyear)
            oRset.DoQuery("SELECT T0.""ObjectCode"", T0.""Series"", T0.""SeriesName""  FROM NNM1 T0 WHERE ""ObjectCode"" = 30 AND T0.""SeriesName"" = '" & sSeries & "'")

            If oRset.RecordCount = 0 Then
                sErr = " Series- " & sSeries & " not defined in SAP "
            ...
            oJournalEntry.ReferenceDate = oDsJournal.Tables(0).Rows(0).Item(3).ToString.Trim
            oJournalEntry.Series = sJESeries
            '' oJournalEntry.DueDate = oDsJournal.Tables(0).Rows(0).Item(3).ToString.Trim
            '' oJournalEntry.TaxDate = oDsJournal.Tables(0).Rows(0).Item(3).ToString.Trim
            '' oJournalEntry.Reference3 = "Daily XN Closing for " & oDsJournal.Tables(0).Rows(0).Item(3).ToString.Trim & ", Tax Invoices: " & oDsJournal.Tables(0).Rows(0).Item(4).ToString.Trim & " - " & oDsJournal.Tables(0).Rows(0).Item(5).ToString.Trim & "   "
            oJournalEntry.Reference = oDsJournal.Tables(0).Rows(0).Item(2).ToString.Trim
            oJournalEntry.Memo = "Daily XN Closing for " & Right(oDsJournal.Tables(0).Rows(0).Item(4).ToString.Trim, 11) & " (" & oDsJournal.Tables(0).Rows(0).Item(4).ToString.Trim & " - " & oDsJournal.Tables(0).Rows(0).Item(5).ToString.Trim & " ) "
            ...
            For mjs As Integer = 0 To oDsJournal.Tables(1).Rows.Count - 1
                ...
                oJournalEntry.Lines.Debit = Math.Abs(CDbl(oDsJournal.Tables(1).Rows(mjs).Item(1).ToString.Trim))
                ...
                dDebitAmount += Math.Abs(CDbl(oDsJournal.Tables(1).Rows(mjs).Item(1).ToString.Trim))
            ...
            Console.WriteLine("Journal Entry DocEntry  " & sDocEntry, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Journal Entry DocEntry  " & sDocEntry, sFuncName)
